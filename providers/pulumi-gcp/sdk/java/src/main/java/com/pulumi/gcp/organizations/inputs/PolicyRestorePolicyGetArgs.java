// *** WARNING: this file was generated by pulumi-java-gen. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.gcp.organizations.inputs;

import com.pulumi.core.Output;
import com.pulumi.core.annotations.Import;
import java.lang.Boolean;
import java.util.Objects;


public final class PolicyRestorePolicyGetArgs extends com.pulumi.resources.ResourceArgs {

    public static final PolicyRestorePolicyGetArgs Empty = new PolicyRestorePolicyGetArgs();

    /**
     * May only be set to true. If set, then the default Policy is restored.
     * 
     */
    @Import(name="default", required=true)
    private Output<Boolean> default_;

    public Output<Boolean> default_() {
        return this.default_;
    }

    private PolicyRestorePolicyGetArgs() {}

    private PolicyRestorePolicyGetArgs(PolicyRestorePolicyGetArgs $) {
        this.default_ = $.default_;
    }

    public static Builder builder() {
        return new Builder();
    }
    public static Builder builder(PolicyRestorePolicyGetArgs defaults) {
        return new Builder(defaults);
    }

    public static final class Builder {
        private PolicyRestorePolicyGetArgs $;

        public Builder() {
            $ = new PolicyRestorePolicyGetArgs();
        }

        public Builder(PolicyRestorePolicyGetArgs defaults) {
            $ = new PolicyRestorePolicyGetArgs(Objects.requireNonNull(defaults));
        }

        public Builder default_(Output<Boolean> default_) {
            $.default_ = default_;
            return this;
        }

        public Builder default_(Boolean default_) {
            return default_(Output.of(default_));
        }

        public PolicyRestorePolicyGetArgs build() {
            $.default_ = Objects.requireNonNull($.default_, "expected parameter 'default' to be non-null");
            return $;
        }
    }

}
