// *** WARNING: this file was generated by pulumi-java-gen. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.gcp.storage.inputs;

import com.pulumi.core.Output;
import com.pulumi.core.annotations.Import;
import java.lang.Integer;
import java.util.Objects;


public final class TransferJobScheduleScheduleStartDateGetArgs extends com.pulumi.resources.ResourceArgs {

    public static final TransferJobScheduleScheduleStartDateGetArgs Empty = new TransferJobScheduleScheduleStartDateGetArgs();

    /**
     * Day of month. Must be from 1 to 31 and valid for the year and month.
     * 
     */
    @Import(name="day", required=true)
    private Output<Integer> day;

    public Output<Integer> day() {
        return this.day;
    }

    /**
     * Month of year. Must be from 1 to 12.
     * 
     */
    @Import(name="month", required=true)
    private Output<Integer> month;

    public Output<Integer> month() {
        return this.month;
    }

    /**
     * Year of date. Must be from 1 to 9999.
     * 
     */
    @Import(name="year", required=true)
    private Output<Integer> year;

    public Output<Integer> year() {
        return this.year;
    }

    private TransferJobScheduleScheduleStartDateGetArgs() {}

    private TransferJobScheduleScheduleStartDateGetArgs(TransferJobScheduleScheduleStartDateGetArgs $) {
        this.day = $.day;
        this.month = $.month;
        this.year = $.year;
    }

    public static Builder builder() {
        return new Builder();
    }
    public static Builder builder(TransferJobScheduleScheduleStartDateGetArgs defaults) {
        return new Builder(defaults);
    }

    public static final class Builder {
        private TransferJobScheduleScheduleStartDateGetArgs $;

        public Builder() {
            $ = new TransferJobScheduleScheduleStartDateGetArgs();
        }

        public Builder(TransferJobScheduleScheduleStartDateGetArgs defaults) {
            $ = new TransferJobScheduleScheduleStartDateGetArgs(Objects.requireNonNull(defaults));
        }

        public Builder day(Output<Integer> day) {
            $.day = day;
            return this;
        }

        public Builder day(Integer day) {
            return day(Output.of(day));
        }

        public Builder month(Output<Integer> month) {
            $.month = month;
            return this;
        }

        public Builder month(Integer month) {
            return month(Output.of(month));
        }

        public Builder year(Output<Integer> year) {
            $.year = year;
            return this;
        }

        public Builder year(Integer year) {
            return year(Output.of(year));
        }

        public TransferJobScheduleScheduleStartDateGetArgs build() {
            $.day = Objects.requireNonNull($.day, "expected parameter 'day' to be non-null");
            $.month = Objects.requireNonNull($.month, "expected parameter 'month' to be non-null");
            $.year = Objects.requireNonNull($.year, "expected parameter 'year' to be non-null");
            return $;
        }
    }

}
