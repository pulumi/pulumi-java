// *** WARNING: this file was generated by pulumi-java-gen. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.azurenative.web.inputs;

import com.pulumi.core.annotations.Import;
import java.lang.String;
import java.util.Objects;


/**
 * Container App Secret.
 * 
 */
public final class ContainerAppSecretResponse extends com.pulumi.resources.InvokeArgs {

    public static final ContainerAppSecretResponse Empty = new ContainerAppSecretResponse();

    /**
     * Secret Name.
     * 
     */
    @Import(name="name", required=true)
    private String name;

    public String name() {
        return this.name;
    }

    /**
     * Secret Value.
     * 
     */
    @Import(name="value", required=true)
    private String value;

    public String value() {
        return this.value;
    }

    private ContainerAppSecretResponse() {}

    private ContainerAppSecretResponse(ContainerAppSecretResponse $) {
        this.name = $.name;
        this.value = $.value;
    }

    public static Builder builder() {
        return new Builder();
    }
    public static Builder builder(ContainerAppSecretResponse defaults) {
        return new Builder(defaults);
    }

    public static final class Builder {
        private ContainerAppSecretResponse $;

        public Builder() {
            $ = new ContainerAppSecretResponse();
        }

        public Builder(ContainerAppSecretResponse defaults) {
            $ = new ContainerAppSecretResponse(Objects.requireNonNull(defaults));
        }

        public Builder name(String name) {
            $.name = name;
            return this;
        }

        public Builder value(String value) {
            $.value = value;
            return this;
        }

        public ContainerAppSecretResponse build() {
            $.name = Objects.requireNonNull($.name, "expected parameter 'name' to be non-null");
            $.value = Objects.requireNonNull($.value, "expected parameter 'value' to be non-null");
            return $;
        }
    }

}
