// *** WARNING: this file was generated by pulumi-java-gen. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.oci.DevOps.inputs;

import com.pulumi.core.Output;
import com.pulumi.core.annotations.Import;
import java.lang.String;
import java.util.List;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;


public final class DeployStageGreenBackendIpsArgs extends com.pulumi.resources.ResourceArgs {

    public static final DeployStageGreenBackendIpsArgs Empty = new DeployStageGreenBackendIpsArgs();

    /**
     * (Updatable) The IP address of the backend server. A server could be a compute instance or a load balancer.
     * 
     */
    @Import(name="items")
    private @Nullable Output<List<String>> items;

    /**
     * @return (Updatable) The IP address of the backend server. A server could be a compute instance or a load balancer.
     * 
     */
    public Optional<Output<List<String>>> items() {
        return Optional.ofNullable(this.items);
    }

    private DeployStageGreenBackendIpsArgs() {}

    private DeployStageGreenBackendIpsArgs(DeployStageGreenBackendIpsArgs $) {
        this.items = $.items;
    }

    public static Builder builder() {
        return new Builder();
    }
    public static Builder builder(DeployStageGreenBackendIpsArgs defaults) {
        return new Builder(defaults);
    }

    public static final class Builder {
        private DeployStageGreenBackendIpsArgs $;

        public Builder() {
            $ = new DeployStageGreenBackendIpsArgs();
        }

        public Builder(DeployStageGreenBackendIpsArgs defaults) {
            $ = new DeployStageGreenBackendIpsArgs(Objects.requireNonNull(defaults));
        }

        /**
         * @param items (Updatable) The IP address of the backend server. A server could be a compute instance or a load balancer.
         * 
         * @return builder
         * 
         */
        public Builder items(@Nullable Output<List<String>> items) {
            $.items = items;
            return this;
        }

        /**
         * @param items (Updatable) The IP address of the backend server. A server could be a compute instance or a load balancer.
         * 
         * @return builder
         * 
         */
        public Builder items(List<String> items) {
            return items(Output.of(items));
        }

        /**
         * @param items (Updatable) The IP address of the backend server. A server could be a compute instance or a load balancer.
         * 
         * @return builder
         * 
         */
        public Builder items(String... items) {
            return items(List.of(items));
        }

        public DeployStageGreenBackendIpsArgs build() {
            return $;
        }
    }

}
