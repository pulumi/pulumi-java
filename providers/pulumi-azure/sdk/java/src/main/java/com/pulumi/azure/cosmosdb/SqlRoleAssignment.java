 ...
 *             .type(&#34;CustomRole&#34;)
 *             .assignableScopes(Output.tuple(exampleResourceGroup.getName(), exampleAccount.getName()).apply(values -&gt; {
 *                 var exampleResourceGroupName = values.t1;
 *                 var exampleAccountName = values.t2;
 *                 return String.format(&#34;/subscriptions/%s/resourceGroups/%s/providers/Microsoft.DocumentDB/databaseAccounts/%s&#34;, current.apply(getClientConfigResult -&gt; getClientConfigResult.getSubscriptionId()),exampleResourceGroupName,exampleAccountName);
 *             }))
 *             .permissions(SqlRoleDefinitionPermission.builder()
 ...
 *             .resourceGroupName(exampleResourceGroup.getName())
 *             .accountName(exampleAccount.getName())
 *             .roleDefinitionId(exampleSqlRoleDefinition.getId())
 *             .principalId(current.apply(getClientConfigResult -&gt; getClientConfigResult.getObjectId()))
 *             .scope(Output.tuple(exampleResourceGroup.getName(), exampleAccount.getName()).apply(values -&gt; {
 *                 var exampleResourceGroupName = values.t1;
 *                 var exampleAccountName = values.t2;
 *                 return String.format(&#34;/subscriptions/%s/resourceGroups/%s/providers/Microsoft.DocumentDB/databaseAccounts/%s&#34;, current.apply(getClientConfigResult -&gt; getClientConfigResult.getSubscriptionId()),exampleResourceGroupName,exampleAccountName);
 *             }))
