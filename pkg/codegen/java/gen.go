...
// should act like we are inside an Output<T>.
	...
	if mod.generatePolicyPack {
		inner = inner.StripOutputAndOptional()
	}
	return inner
}
...
func (pt *plainType) genPolicyProperty(ctx *classFileContext, prop *schema.Property, targetType TypeShape) error {
	...
	targetTypeNoAnnotations := targetType
	targetTypeNoAnnotations.Annotations = nil

	wrappedTargetType := TypeShape{
		Type:        names.UndeferrableValue,
		Parameters:  []TypeShape{targetTypeNoAnnotations},
		Annotations: targetType.Annotations,
	}

	fprintf(w, "    private %s %s;\n\n", wrappedTargetType.ToCode(ctx.imports), fieldName)
	fprintf(w, "    public %s %s() {\n", targetType.ToCode(ctx.imports), fieldName)
	fprintf(w, "        if (%s == null) return null;\n", fieldName)
	fprintf(w, "        return %s.getValue(\"%s.%s\");\n", fieldName, ctx.className, fieldName)
	fprintf(w, "    }\n\n")
