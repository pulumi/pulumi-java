...
func (mod *modContext) getUtilitiesImport() string {
	return normalizeJavaName(mod.rootPackageName) + ".Utilities"
}
...
func (mod *modContext) genHeader(w io.Writer, imports []string, qualifier string) {
	...
	_, _ = fmt.Fprintf(w, "\n")
	_, _ = fmt.Fprintf(w, "package %s%s;\n", normalizeJavaName(mod.packageName), normalizeJavaName(qualifier))
	_, _ = fmt.Fprintf(w, "\n")
...
func (mod *modContext) genUtilities() (string, error) {
	...
	err := jvmUtilitiesTemplate.Execute(w, jvmUtilitiesTemplateContext{
		...
		PackageName: normalizeJavaName(mod.packageName),
...
func (mod *modContext) gen(fs fs) error {
	...
	for _, r := range mod.resources {
		...
		mod.genHeader(argsBuffer, mod.getPulumiImports(), "")
		if err := args.genInputType(argsBuffer); err != nil {
			return err
		}
		addFile(resourceName(r)+"Args.java", argsBuffer.String())
