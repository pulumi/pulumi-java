// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.kebabnames.kebabmodule;

import com.pulumi.core.Output;
import com.pulumi.core.annotations.Import;
import com.pulumi.exceptions.MissingRequiredPropertyException;
import com.pulumi.kebabnames.kebabmodule.inputs.NestedInputArgs;
import java.lang.Boolean;
import java.util.Objects;


public final class SomeResourceArgs extends com.pulumi.resources.ResourceArgs {

    public static final SomeResourceArgs Empty = new SomeResourceArgs();

    @Import(name="nested", required=true)
    private Output<NestedInputArgs> nested;

    public Output<NestedInputArgs> nested() {
        return this.nested;
    }

    @Import(name="theInput", required=true)
    private Output<Boolean> theInput;

    public Output<Boolean> theInput() {
        return this.theInput;
    }

    private SomeResourceArgs() {}

    private SomeResourceArgs(SomeResourceArgs $) {
        this.nested = $.nested;
        this.theInput = $.theInput;
    }

    public static Builder builder() {
        return new Builder();
    }
    public static Builder builder(SomeResourceArgs defaults) {
        return new Builder(defaults);
    }

    public static final class Builder {
        private SomeResourceArgs $;

        public Builder() {
            $ = new SomeResourceArgs();
        }

        public Builder(SomeResourceArgs defaults) {
            $ = new SomeResourceArgs(Objects.requireNonNull(defaults));
        }

        public Builder nested(Output<NestedInputArgs> nested) {
            $.nested = nested;
            return this;
        }

        public Builder nested(NestedInputArgs nested) {
            return nested(Output.of(nested));
        }

        public Builder theInput(Output<Boolean> theInput) {
            $.theInput = theInput;
            return this;
        }

        public Builder theInput(Boolean theInput) {
            return theInput(Output.of(theInput));
        }

        public SomeResourceArgs build() {
            if ($.nested == null) {
                throw new MissingRequiredPropertyException("SomeResourceArgs", "nested");
            }
            if ($.theInput == null) {
                throw new MissingRequiredPropertyException("SomeResourceArgs", "theInput");
            }
            return $;
        }
    }

}
