...
func TestLanguage(t *testing.T) {
	...
	prepare, err := engine.PrepareLanguageTests(context.Background(), &testingrpc.PrepareLanguageTestsRequest{
		...
		CoreSdkDirectory:     "../../../sdk",
		CoreSdkVersion:       sdk.Version.String(),
		SnapshotEdits: []*testingrpc.PrepareLanguageTestsRequest_Replacement{
			{
				Path:        "build.gradle",
				Pattern:     fmt.Sprintf("com.pulumi:pulumi:%s", sdk.Version.String()),
				Replacement: "com.pulumi:pulumi:CORE.VERSION",
			},
		},
	})
	require.NoError(t, err)
...
var expectedFailures = map[string]string{
	//"l1-empty":                           "unimplemented for Java",
